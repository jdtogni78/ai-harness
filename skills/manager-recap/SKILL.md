...
name: manager-recap
description: >-
  Manager activity recap (formerly the `report` skill; renamed in #179 when the
  report-v2 review renderer took the `report` name). The reporter the manager
  delegates to ("reporter, tell me what happened"). Given
...
# /manager-recap — manager activity reporter

> Renamed from `/report` in #179. Behaviour is unchanged; only the skill name
> moved (the `report` name now belongs to the report-v2 review renderer).
...
```bash
python3 skills/manager-recap/scripts/report.py [manager_cse_id] \
