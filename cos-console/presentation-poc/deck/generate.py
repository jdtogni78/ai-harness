...
# --------------------------------------------------------------------------- #
# generic report deck — reused by the `report` skill (manager activity recap).
# Same template chrome, same narration/demo contract; the only difference is a
# pre-rendered `slides` list instead of the StatusReport builders. Import these
# (do NOT copy the renderer):
#     import sys; sys.path.insert(0, ".../presentation-poc")
#     from deck.generate import write_generic_deck
# `deck` shape:
#     {"title": str, "subtitle": str?, "generated_at": str?, "source": str?,
#      "slides": [ {"widget": str, "kicker": str?, "title": str?,
#                   "heading": str?, "subtitle": str?, "html": str,
#                   "narration": str?} ]}
# --------------------------------------------------------------------------- #
def render_generic_html(deck: dict) -> str:
    """Inject a generic `slides` deck into the shared template."""
    blob = json.dumps(deck, ensure_ascii=False, separators=(",", ":"))
    tpl = TEMPLATE.read_text()
    title = deck.get("title", "report deck")
    return tpl.replace("__TITLE__", title).replace("__DECK_DATA__", blob)


def generic_narration_companion(deck: dict) -> dict:
    """Per-slide {widget, narration} — same contract as narration_companion,
    driven off the generic slide list. Honors anti-fabrication: whatever the
    caller put in each slide's `narration` is voiced verbatim (a 'no tests run'
    line stays a 'no tests run' line)."""
    return {
        "title": deck.get("title"),
        "generated_at": deck.get("generated_at"),
        "source": deck.get("source", "report"),
        "slides": [{"index": n, "widget": s.get("widget", f"slide{n}"),
                    "narration": s.get("narration", "")}
                   for n, s in enumerate(deck.get("slides", []))],
    }


def generic_demo_yaml(deck: dict, deck_url: str) -> str:
    """narrate-demo talk-over script for a generic deck — one `goto <deck>#N`
    step per slide, spoken line = that slide's narration. Mirrors
    build_demo_yaml but over `deck['slides']`."""
    title = deck.get("title", "report")
    slides = deck.get("slides", [])
    lines = [
        f"# Generated by deck.generate (generic) — narrate-demo talk-over script.",
    ...
    for n, s in enumerate(slides):
        lines.append(f"  - say: {_yaml_dq(s.get('narration',''))}")
        lines.append("    do: goto")
        lines.append(f"    url: {_yaml_dq(deck_url + '#' + str(n))}")
    lines.append('  - say: "That\'s the recap — ask me for any section in detail."')
    lines.append("    do: wait")
    lines.append("    ms: 900")
    return "\n".join(lines) + "\n"


def write_generic_deck(deck: dict, outdir="out", slug="report") -> dict:
    """Write <slug>.html + <slug>.narration.json + <slug>.demo.yaml for a
    generic slide deck. Returns the written paths. Self-contained HTML, no deps."""
    out = Path(outdir)
    out.mkdir(parents=True, exist_ok=True)
    html_path = out / f"{slug}.html"
    narr_path = out / f"{slug}.narration.json"
    demo_path = out / f"{slug}.demo.yaml"
    html_path.write_text(render_generic_html(deck))
    narr_path.write_text(json.dumps(generic_narration_companion(deck),
                                    indent=2, ensure_ascii=False))
    demo_path.write_text(generic_demo_yaml(deck, html_path.resolve().as_uri()))
    return {"html": html_path, "narration": narr_path, "demo": demo_path}
